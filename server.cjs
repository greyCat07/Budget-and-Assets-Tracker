...
var cloudBackups = /* @__PURE__ */ new Map();
async function startServer() {
  ...
  app.get("/api/market/quotes", (req, res) => {
    const now = Date.now();
    const jitter = (seed, amp = 4e-3) => Math.sin(now / 15e3 + seed) * amp;
    const stocks = [
      {
        symbol: "AAPL",
        name: "Apple Inc.",
      ...
      },
      {
        symbol: "NVDA",
        name: "NVIDIA Corp.",
        price: +(128.9 * (1 + jitter(2))).toFixed(2),
        change24h: +(3.85 + jitter(2, 0.8)).toFixed(2),
        changePercent: +(3.08 + jitter(2, 0.6)).toFixed(2),
        marketCap: "3.16T",
        sparkline: [122.4, 124, 123.5, 126.8, 127.4, 128.2, +(128.9 * (1 + jitter(2))).toFixed(2)],
        type: "stock"
      },
      {
        symbol: "MSFT",
        name: "Microsoft Corp.",
        price: +(448.2 * (1 + jitter(3))).toFixed(2),
        change24h: +(-1.15 + jitter(3, 0.4)).toFixed(2),
        changePercent: +(-0.26 + jitter(3, 0.1)).toFixed(2),
        marketCap: "3.33T",
        sparkline: [450.1, 449.5, 451.2, 447.8, 448.5, 447.9, +(448.2 * (1 + jitter(3))).toFixed(2)],
        type: "stock"
      },
      {
        symbol: "SPY",
        name: "SPDR S&P 500 ETF",
        price: +(562.8 * (1 + jitter(4))).toFixed(2),
        change24h: +(2.1 + jitter(4, 0.3)).toFixed(2),
        changePercent: +(0.37 + jitter(4, 0.1)).toFixed(2),
        marketCap: "560B",
        sparkline: [558.2, 559.4, 560.1, 561.3, 561.8, 562.5, +(562.8 * (1 + jitter(4))).toFixed(2)],
        type: "etf"
      },
      {
        symbol: "TSLA",
        name: "Tesla Inc.",
        price: +(214.3 * (1 + jitter(5))).toFixed(2),
        change24h: +(5.2 + jitter(5, 1.2)).toFixed(2),
        changePercent: +(2.49 + jitter(5, 0.5)).toFixed(2),
        marketCap: "682B",
        sparkline: [205.4, 207.2, 209.1, 211.5, 210.8, 213.6, +(214.3 * (1 + jitter(5))).toFixed(2)],
        type: "stock"
      }
    ];
    const cryptos = [
      {
        symbol: "BTC",
        name: "Bitcoin",
        price: +(64250 * (1 + jitter(10, 8e-3))).toFixed(2),
        change24h: +(1420 + jitter(10, 300)).toFixed(2),
        changePercent: +(2.26 + jitter(10, 0.4)).toFixed(2),
        marketCap: "1.26T",
        sparkline: [61800, 62400, 63100, 62800, 63900, 64100, +(64250 * (1 + jitter(10, 8e-3))).toFixed(2)],
        type: "crypto"
      },
      {
        symbol: "ETH",
        name: "Ethereum",
        price: +(3480.5 * (1 + jitter(11, 0.01))).toFixed(2),
        change24h: +(85.2 + jitter(11, 15)).toFixed(2),
        changePercent: +(2.51 + jitter(11, 0.4)).toFixed(2),
        marketCap: "418B",
        sparkline: [3350, 3380, 3420, 3390, 3450, 3470, +(3480.5 * (1 + jitter(11, 0.01))).toFixed(2)],
        type: "crypto"
      },
      {
        symbol: "SOL",
        name: "Solana",
        price: +(154.2 * (1 + jitter(12, 0.015))).toFixed(2),
        change24h: +(7.4 + jitter(12, 2)).toFixed(2),
        changePercent: +(5.04 + jitter(12, 1)).toFixed(2),
        marketCap: "71.8B",
        sparkline: [142.1, 145.6, 147.2, 149.8, 151.2, 153.5, +(154.2 * (1 + jitter(12, 0.015))).toFixed(2)],
        type: "crypto"
      },
      {
        symbol: "ADA",
        ...
        change24h: +(0.012 + jitter(13, 5e-3)).toFixed(4),
        changePercent: +(2.9 + jitter(13, 0.8)).toFixed(2),
        marketCap: "15.2B",
